...
namespace CustomLogic
{
    class CustomLogicEvaluator
    {
        ...
        public void OnPlayerLeave(PhotonPlayer player)
        {
            var playerBuiltin = new CustomLogicPlayerBuiltin(player);
            foreach (var instance in _callback)
        ...
        public void OnNetworkMessage(PhotonPlayer sender, string message)
        {
        ...
        private IEnumerator OnSecond()
        {
            while (true)
            {
                ...
                {
                    RPCManager.PhotonView.RPC("SyncCurrentTimeRPC", PhotonTargets.Others, new object[] { CurrentTime });
