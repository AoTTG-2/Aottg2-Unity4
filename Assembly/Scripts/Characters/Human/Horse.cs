...
namespace Characters
{
    class Horse: BaseCharacter
    {
        ...
        protected override void Awake()
        {
            ...
            _dust = transform.Find("Dust").GetComponent<ParticleSystem>();
            _dust.Play();
            ToggleDust(false);
        }
        ...
        {
            if (toggle && !_dust.enableEmission)
                _dust.enableEmission = true;
            else if (!toggle && _dust.enableEmission)
                _dust.enableEmission = false;
        }
        ...
        private void UpdateIdle()
        {
            ...
            if (_idleTimeLeft > 0f)
                return;
            if (!Cache.Animation.IsPlaying("horse_idle0"))
            {
                CrossFade("horse_idle0", 0.1f);
            ...
            if (choose < 0.25f)
                IdleOneShot("horse_idle1");
            else if (choose < 0.5f)
                IdleOneShot("horse_idle2");
            else if (choose < 0.75f)
                IdleOneShot("horse_idle3");
            else
                IdleOneShot("horse_crazy");
        }
        ...
        protected override void LateUpdate()
        {
            ...
            if (IsMine())
            {
                ...
                {
                    CrossFadeIfNotPlaying("horse_Run", 0.1f);
                    if (_owner.MountState == HumanMountState.Horse)
                        _owner.CrossFadeIfNotPlaying("horse_run", 0.1f);
                    _idleTimeLeft = 0f;
                }
                else if (Cache.Rigidbody.velocity.magnitude > 1f)
                {
                    CrossFadeIfNotPlaying("horse_WALK", 0.1f);
                    if (_owner.MountState == HumanMountState.Horse)
                        _owner.CrossFadeIfNotPlaying("horse_idle", 0.1f);
                    _idleTimeLeft = 0f;
                }
                else
                {
            ...
            if (Cache.Animation.IsPlaying("horse_Run"))
                ToggleDust(true);
            else
                ToggleDust(false);
        }
