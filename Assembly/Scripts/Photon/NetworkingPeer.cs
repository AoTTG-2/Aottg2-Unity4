...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    public void checkLAN()
    {
        ...
        {
            mGameserver = MasterServerAddress.Split(new char[] { ':' })[0] + ":" + mGameserver.Split(new char[] { ':' })[1];
        }
    }
    ...
    private void DisconnectToReconnect2()
    {
        ...
        {
            case ServerConnection.MasterServer:
                this.State = ReturnPeerState(2);
                base.Disconnect();
                break;

            case ServerConnection.GameServer:
                this.State = ReturnPeerState(3);
                base.Disconnect();
                break;

            case ServerConnection.NameServer:
                this.State = ReturnPeerState(4);
                base.Disconnect();
                break;
        }
    }
    ...
    internal GameObject DoInstantiate2(ExitGames.Client.Photon.Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
    {
        ...
        if (!AnticheatManager.CheckPhotonEvent(photonPlayer, PhotonEventType.Instantiate, new object[] { key, numArray }))
        {
            return null;
        }
        ...
        if (resourceGameObject == null)
        {
            if (!UsePrefabCache || !PrefabCache.TryGetValue(key, out resourceGameObject))
            {
                ...
                {
                    resourceGameObject = (GameObject)AssetBundleManager.LoadAsset(key.Substring(8));
                }
                else
                {
        ...
        switch (photonEvent.Code)
        {
                    ...
                    if (((sender == null) || !sender.isMasterClient) || (num10 != sender.ID))
                    {
                        if (!(((sender == null) || sender.isMasterClient) || sender.isLocal))
                        {
                            if (PhotonNetwork.isMasterClient)
                            {
                                ...
                                AnticheatManager.KickPlayer(sender, false, "Stealing MC");
                            }
                            return;
                        }
            ...
            case 0xe4:
                ...
                if (this.mQueuePosition == 0)
                {
                    ...
                    {
                        this.State = ReturnPeerState(0);
                        this.OpJoinLobby(this.lobby);
                    }
                    else
                    {
                        this.State = ReturnPeerState(1);
    ...
    private PeerStates ReturnPeerState(int peerstate)
    {
        switch (peerstate)
        {
            case 0:
