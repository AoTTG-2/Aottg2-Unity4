...
namespace UI
{
    class EmoteHandler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            for (int i = 0; i < 5; i++)
            {
                EmoteTextPopup emoteTextPopup = ElementFactory.InstantiateAndSetupPanel<EmoteTextPopup>(transform, "EmoteTextPopup").GetComponent<EmoteTextPopup>();
                _emoteTextPopups.Add(emoteTextPopup);
                EmoteEmojiPopup emoteEmojiPopup = ElementFactory.InstantiateAndSetupPanel<EmoteEmojiPopup>(transform, "EmoteEmojiPopup").GetComponent<EmoteEmojiPopup>();
        ...
        {
            if (UIManager.CurrentMenu == null || !SettingsManager.UISettings.ShowEmotes.Value || InGameManager.MuteEmote.Contains(info.sender.ID))
                return;
