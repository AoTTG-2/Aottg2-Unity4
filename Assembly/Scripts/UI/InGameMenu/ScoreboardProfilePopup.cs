    ...
    class ScoreboardProfilePopup: PromptPopup
    {
        ...
        public void Show(PhotonPlayer player)
        {
            ...
            foreach (var item in _items)
                Destroy(item);
            _items.Clear();
            var playerInfo = InGameManager.AllPlayerInfo[player.ID].Profile;
            _profile = playerInfo;
            ...
            ElementFactory.CreateRawImage(group, style, UIManager.GetProfileIcon(_profile.ProfileIcon.Value), 256, 256);
