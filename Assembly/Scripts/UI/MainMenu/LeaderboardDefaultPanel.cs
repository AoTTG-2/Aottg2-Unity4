...
namespace UI
{
    class LeaderboardDefaultPanel: CategoryPanel
    {
        ...
        public override void Setup(BasePanel parent = null)
        {
            ...
            if (PastebinLoader.Status == PastebinStatus.Failed)
            {
        ...
        private void CreateRank(Transform parent, ElementStyle style, int rank)
        {
            if (rank <= 3)
            {
                RawImage img = ElementFactory.CreateRawImage(parent, style, "TrophyIcon", 32f, 32f).GetComponent<RawImage>();
                img.color = UIManager.GetThemeColor(ThemePanel, "Rank", "Trophy" + rank.ToString() + "Color");
            }
            else if (rank <= 10)
            {
                RawImage img = ElementFactory.CreateRawImage(parent, style, "Badge1Icon", 32f, 32f).GetComponent<RawImage>();
