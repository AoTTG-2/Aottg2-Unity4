...
namespace UI
{
    class QuestCategoryPanel: CategoryPanel
    {
        ...
        protected void CreateQuestItems(List<QuestItem> items)
        {
            ...
            {
                Transform panel = ElementFactory.InstantiateAndBind(SinglePanel, "QuestItemPanel").transform;
                ...
                panel.Find("Panel/Icon").GetComponent<RawImage>().texture = (Texture2D)AssetBundleManager.LoadAsset(item.Icon.Value + "Icon", cached: true);
                SetTitle(item, panel);
