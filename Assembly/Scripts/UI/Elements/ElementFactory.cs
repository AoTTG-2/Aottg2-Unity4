...
namespace UI
{
    class ElementFactory: MonoBehaviour
    {
        ...
        {
            return CreateMenu<T>("DefaultMenu");
        }

        public static T CreateMenu<T>(string asset) where T : BaseMenu
        {
            GameObject menu = AssetBundleManager.InstantiateAsset<GameObject>(asset, cached: true);
        ...
        {
            return InstantiateAndSetupPanel<T>(parent, "EmptyPanel", enabled).GetComponent<T>();
        }

        public static GameObject CreateEmptyPanel(Transform parent, Type t, bool enabled = false)
        {
            GameObject panel = InstantiateAndBind(parent, "EmptyPanel");
        ...
        {
            return InstantiateAndSetupPanel<T>(parent, "SimplePanel", enabled).GetComponent<T>();
        }

        public static T CreateDefaultPopup<T>(Transform parent, bool enabled = false) where T : BasePopup
        {
            return InstantiateAndSetupPanel<T>(parent, "HeadedPanel", enabled).GetComponent<T>();
        }
        ...
        {
            return InstantiateAndSetupPanel<TooltipPopup>(parent, "TooltipPopup", enabled);
        }

        public static TipPanel CreateTipPanel(Transform parent, bool enabled = false)
        {
            GameObject tip = InstantiateAndBind(parent, "TipPanel");
        ...
        public static GameObject CreateIconButton(Transform parent, ElementStyle style, string icon, float elementWidth = 32f, float elementHeight = 32f, 
            UnityAction onClick = null)
        {
            GameObject button = InstantiateAndBind(parent, "IconButton");
            LayoutElement layout = button.GetComponent<LayoutElement>();
            layout.preferredWidth = elementWidth;
            layout.preferredHeight = elementHeight;
            if (onClick != null)
            ...
            img.texture = (Texture2D)AssetBundleManager.LoadAsset(icon, true);
        ...
        {
            GameObject obj = InstantiateAndBind(parent, "RawImage");
            RawImage rawImage = obj.GetComponent<RawImage>();
            LayoutElement layout = obj.GetComponent<LayoutElement>();
            layout.preferredWidth = elementWidth;
            layout.preferredHeight = elementHeight;
            if (image != string.Empty)
                rawImage.texture = (Texture2D)AssetBundleManager.LoadAsset(image, true);
            return obj;
        }
        ...
        {
            GameObject obj = CreateRawImage(parent, style, "TooltipIcon", elementWidth, elementHeight);
        ...
        public static GameObject CreateHUDLabel(Transform parent, ElementStyle style, string title, FontStyle fontStyle = FontStyle.Normal,
            TextAnchor alignment = TextAnchor.MiddleCenter)
        {
            GameObject label = InstantiateAndBind(parent, "HUDLabel");
            Text text = label.GetComponent<Text>();
            text.fontSize = style.FontSize;
            text.text = title;
            text.fontStyle = fontStyle;
        ...
        {
            GameObject vectorSetting = InstantiateAndBind(parent, "Vector3Setting");
