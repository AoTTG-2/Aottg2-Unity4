...
namespace UI
{
    class MapEditorAddObjectPanel : CategoryPanel
    {
        ...
        protected void CreateRow(List<string> items)
        {
            ...
            {
                var obj = ElementFactory.InstantiateAndBind(group.transform, "MapEditorObjectButton");
                obj.GetComponent<Button>().onClick.AddListener(() => OnSelectObject(item));
                string preview = GetPreviewName(item);
                if (AssetBundleManager.MainAssetBundle.Contains(preview))
                {
                    var texture = (Texture2D)AssetBundleManager.LoadAsset(preview);
