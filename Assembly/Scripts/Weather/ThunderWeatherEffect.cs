    ...
    class ThunderWeatherEffect : BaseWeatherEffect
    {
        ...
        public static void OnFinishInit()
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                {
                    LightningParticle particle = AssetBundleManager.InstantiateAsset<GameObject>("LightningParticle").AddComponent<LightningParticle>();
                    Util.DontDestroyOnLoad(particle.gameObject);
